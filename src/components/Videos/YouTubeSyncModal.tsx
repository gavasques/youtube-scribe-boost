...
export function YouTubeSyncModal({ open, onClose, onSyncComplete }: YouTubeSyncModalProps) {
  ...
  const { syncWithYouTube, syncAllVideos, syncing, progress, batchSync, pauseBatchSync, resumeBatchSync, stopBatchSync } = useYouTubeSync()
  ...
  return (
    <>
      <Dialog open={open && !showProgress} onOpenChange={onClose}>
      ...
      <SyncProgressModal
        open={showProgress && (syncing || !!progress)}
